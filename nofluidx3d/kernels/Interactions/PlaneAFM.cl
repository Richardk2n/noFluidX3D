kernel void Interaction_PlaneAFMO(volatile global ibmPrecisionFloat* particleForce, const global ibmPrecisionFloat* points, const ibmPrecisionFloat topWallPos, const ibmPrecisionFloat bottomWallPos, const ibmPrecisionFloat forceConst){
...
kernel void Interaction_PlaneAFM(volatile global ibmPrecisionFloat* particleForce, const global ibmPrecisionFloat* points, const ibmPrecisionFloat topWallPos, const ibmPrecisionFloat bottomWallPos, const ibmPrecisionFloat forceConst){
	...
	ibmPrecisionFloat force = pref*(pow(disBottom+1, 1) - pow(disTop+1, 1));
	particleForce[INSERT_NUM_POINTS + pointID] += force;
}
