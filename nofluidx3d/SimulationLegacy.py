...
class Simulation:
    ...
    def initCheckPointVTK(self):
        ...
        self.points.arrayC = cellNew.points.reshape(3 * self.numPoints, order="F").astype(
            np.float64
        )
    ...
    def setShellEllipsoidalReferenceState(self, eccentricity, heteroFunc):
        ...
        self.cell.edgeVectors = (
            heteroFunc(tetraRadiiNorm)[:, np.newaxis] * shellReferenceCell.edgeVectors
            + (1.0 - heteroFunc(tetraRadiiNorm))[:, np.newaxis] * self.cell.edgeVectors
        )
        self.cell.volumes = (
            1.0
            / 6.0
    ...
    def setInteractionSphere(self, record=True, isSpherical=True):
        ...
        interSphere = inter.InteractionSphere(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
    ...
    def setInteractionRoundTip(self, record=True, isSpherical=True):
        ...
        forceConst = self.parameters["PotentialForceConst"]
        interSphere = inter.InteractionRoundTip(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
    ...
    def setInteractionSphereIntegral(self, record=True, isSpherical=True):
        ...
        forceConst = self.parameters["PotentialForceConst"]
        interSphere = inter.InteractionSphereIntegral(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
        ...
        interSphereRecord = inter.InteractionSphereIntegral(
            self.numPoints,
            self.numTetra,
            forceRecord.buf,
            self.points.buf,
    ...
    def setInteractionTipIntegral(self, record=True, isSpherical=True):
        ...
        interTip = inter.InteractionTipIntegral(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
        ...
        # this records the force exerted onto the sphere by the cell
        forceRecord = Bridge(np.zeros(self.numPoints * 3), datatype=np.float64)
        interTipRecord = inter.InteractionTipIntegral(
            self.numPoints,
            self.numTetra,
            forceRecord.buf,
            self.points.buf,
        ...
        def sphereFunc(time):
            if time <= self.parameters["MoveTimeSI"] / self.T0:
                return (
                    ...
                    + self.parameters["SphereRadiusSI"] / self.L0
    ...
    def setInteractionTiltedPlane(self, record=True, isSpherical=True):
        ...
        if isSpherical:
            planeStartingPosition = (
                self.parameters["CELL"]["RadiusSIM"] + self.parameters["InitialDistance"]
            ) / np.sin(np.pi / 2.0 - planeAngle)
        else:
            # calculate closest point to plate
            normal = np.array([np.sin(planeAngle), -np.cos(planeAngle), 0])
            closestPoint = self.cell.points[
                np.einsum("ij, j -> i", self.cell.points, normal).argmin()
            ]
        ...
        def positionFunc(time):
            if time <= self.parameters["MoveTimeSI"] / self.T0:
                return planeStartingPosition - self.parameters["VelocitySI"] / self.V0 * time
            else:
                return (
                    planeStartingPosition
                    - self.parameters["VelocitySI"]
        ...
        forceConst = self.parameters["PotentialForceConst"]
        interTP = inter.InteractionTiltedPlane(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
        ...
        def forceSI():
            ...
            normal = -np.cos(planeAngle) * np.array([0, 1, 0]) + np.sin(planeAngle) * np.array(
                [1, 0, 0]
            )
    ...
    def setInteractionHalfPlane(self, isBelow=True, isSpherical=True):
        ...
        forceConst = self.parameters["PotentialForceConst"]
        interPlane = inter.InteractionHalfPlane(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
    ...
    def setInteractionAdhesivePlane(self, isSpherical=True):
        ...
        pointIsOnSurface = np.isin(np.arange(len(self.cell.mesh.points)), surfaceIndices).astype(
            int
        )
        ...
        interPlane = inter.InteractionAdhesivePlane(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
    ...
    def setInteractionAdhesivePlaneSurfaceIntegral(self, isSpherical=True):
        ...
        adhesionConst = (
            self.parameters["AdhesionConstantSI"]
            / self.E0
            * self.L0**2
            / self.parameters["ReynoldScaling"]
            / self.parameters["CELL"]["Youngsscaling"]
        ...
        distance = self.parameters["AdhesivePlaneDistance"]
        print(f"+++ Adhesionconstant in simulation units: {adhesionConst}")
        interPlane = inter.InteractionAdhesivePlaneSurfaceIntegral(
            self.numPoints,
            self.numTetra,
            self.numTriangles,
            self.force.buf,
    ...
    def setInteractionLinearElastic(self):
        ...
        interLE = inter.InteractionLinearElastic(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
    ...
    def setInteractionLinearPoroElastic(self):
        ...
        interLE = inter.InteractionLinearElasticDeviatoric(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
    ...
    def setInteractionNeoHookean(self, heteroFunction=None):
        ...
        if heteroFunction:
            tetraMiddles = self.cell.points[self.cell.tetras].sum(axis=1) / 4.0
            shearModulus = shearModulus * heteroFunction(
                np.linalg.norm(tetraMiddles, axis=-1) / self.cell.radius
            )
            bulkModulus = bulkModulus * heteroFunction(
                np.linalg.norm(tetraMiddles, axis=-1) / self.cell.radius
            )
            youngsModuli = youngsModulusSI * heteroFunction(
                np.linalg.norm(tetraMiddles, axis=-1) / self.cell.radius
            )
    ...
    def setInteractionSecondOrderNeoHookean(self):
        ...
        bulkModulus = youngsModulus / (3 * (1 - 2 * poissonRatio))
        interNH = inter.InteractionSecondOrderNeoHookean(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
    ...
    def setInteractionPoroNeoHookean(self):
        ...
        interPNH = inter.InteractionPoroNeoHookean(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
    ...
    def setInteractionFiniteStrainViscoplastic(self):
        ...
        flowRate = self.T0
        yieldStress = (
            timeConstantSI ** (1.0 / flowExponent) / self.parameters["CELL"]["YoungsScaling"]
        )
    ...
    def setInteractionCOMForce(self):
        ...
        interCalcCOM = inter.InteractionCalcCOM(
            self.numPoints, self.numTetra, self.points.buf, self.com.buf
        )
        interCOMForce = inter.InteractionCOMForce(
            self.numPoints,
            self.numTetra,
            self.force.buf,
            self.points.buf,
    ...
    def setInteractionPointZeroForce(self):
        springConst = self.parameters["COMSpringConst"]
        interPZF = inter.InteractionPointZeroForce(
    ...
    def setInteractionVelocityVerlet2(self, damping=0.1):
        ...
        interVV = inter.InteractionVelocityVerlet2(
            self.numPoints, self.numTetra, self.force.buf, self.points.buf, self.masses.buf, damping
        )
    ...
    def setInteractionOverDamped(self, point3fix):
        interOD = inter.InteractionOverDamped(
            self.numPoints, self.numTetra, self.force.buf, self.points.buf, point3fix=point3fix
        )
