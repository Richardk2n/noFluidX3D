kernel void Interaction_PoroNeoHookean(volatile global ibmPrecisionFloat* particleForce, const global ibmPrecisionFloat* points, const global uint* tetras, const global ibmPrecisionFloat* referenceEdgeVectors, const global ibmPrecisionFloat* referenceVolumes, const global ibmPrecisionFloat* shearModulus, const global ibmPrecisionFloat* volumeFraction, volatile global ibmPrecisionFloat* vonMises, volatile global ibmPrecisionFloat* pressure){
	...
	const ibmPrecisionFloat eta = (1 + (J-1) / volumeFraction[tetraID]);

	const ibmPrecisionFloat a = shearModulus[tetraID] / 2.0 * ( 4.0/J - 2.0/(J*J) - 2.0*(volumeFraction[tetraID] + 2.0*(J-1.0))/(pow(J, 5.0/3.0) * pow(eta, 1.0/3.0)));
	const ibmPrecisionFloat b = shearModulus[tetraID] / 2.0 * trB * ( 1.0/(J*J) - 1.0/(3.0*pow(J, 2.0/3.0) * pow(eta, 1.0/3.0)) * ((4.0-volumeFraction[tetraID])*eta + (1.0-volumeFraction[tetraID]))/(volumeFraction[tetraID]*eta*eta + (1.0-volumeFraction[tetraID])*eta));
	// Cauchy Stress tensor Poro hyperelastic material Danielsson, Parks, Boyce, 2002
	const ibmPrecisionFloat3 sigma1 = a * B1 + b * E1;
	const ibmPrecisionFloat3 sigma2 = a * B2 + b * E2;
	const ibmPrecisionFloat3 sigma3 = a * B3 + b * E3;
