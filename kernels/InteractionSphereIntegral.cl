...
kernel void Interaction_SphereIntegral(volatile global ibmPrecisionFloat* particleForce, const global ibmPrecisionFloat* points, const global uint* tetras, const ibmPrecisionFloat radius, const ibmPrecisionFloat spherePos, const ibmPrecisionFloat forceConst){
	...
	const ibmPrecisionFloat3 xi1 = (ibmPrecisionFloat3)(beta, alpha, beta);	
	const ibmPrecisionFloat3 xi1 = (ibmPrecisionFloat3)(beta, beta, alpha);	
	const ibmPrecisionFloat3 xi1 = (ibmPrecisionFloat3)(beta, beta, beta);	

	const ibmPrecisionFloat x1 = xi1.x * p1 + xi1.y * p2 + xi1.z * p3 + (1.0 - xi1.x - xi1.y -xi1.z) * p4;
	const ibmPrecisionFloat x2 = xi2.x * p1 + xi2.y * p2 + xi2.z * p3 + (1.0 - xi2.x - xi2.y -xi2.z) * p4;
	const ibmPrecisionFloat x3 = xi3.x * p1 + xi3.y * p2 + xi3.z * p3 + (1.0 - xi3.x - xi3.y -xi3.z) * p4;
	const ibmPrecisionFloat x4 = xi4.x * p1 + xi4.y * p2 + xi4.z * p3 + (1.0 - xi4.x - xi4.y -xi4.z) * p4;
	
	// force density at integration points
	const ibmPrecisionFloat f1 = calcForceSphere(x1, radius, spherePos, forceConst);
	const ibmPrecisionFloat f2 = calcForceSphere(x2, radius, spherePos, forceConst);
	const ibmPrecisionFloat f3 = calcForceSphere(x3, radius, spherePos, forceConst);
	const ibmPrecisionFloat f4 = calcForceSphere(x4, radius, spherePos, forceConst);
